...
static int __kvm_pgtable_walk(struct kvm_pgtable_walk_data *data,
			kvm_pte_t *pgtable, u32 level, u64 ghost_va_partial);

static inline int __kvm_pgtable_visit(struct kvm_pgtable_walk_data *data,
				kvm_pte_t *ptep, u32 level, u64 ghost_va_partial)
{
	int ret = 0;
	u64 addr = data->addr;
	...
	ret = __kvm_pgtable_walk(data, childp, level + 1, ghost_va_partial);
	if (ret)
		goto out;
			...
			kvm_pte_t *pgtable, u32 level, u64 ghost_va_partial)
{
	...
	for (idx = kvm_pgtable_idx(data, level); idx < PTRS_PER_PTE; ++idx) {
		...
		ret = __kvm_pgtable_visit(data, ptep, level, ghost_va_partial_new);
		if (ret)
			break;
	}
...
static int _kvm_pgtable_walk(struct kvm_pgtable_walk_data *data)
{
	...
	for (idx = kvm_pgd_page_idx(data); data->addr < data->end; ++idx) {
		...
		ret = __kvm_pgtable_walk(data, ptep, pgt->start_level, ghost_va_partial);
		if (ret)
			break;
	}
			      ...
			      kvm_pgtable_force_pte_cb_t force_pte_cb)
{
	...
	// GHOST
	mapping_empty(&pgt->ghost_mapping);
	// /GHOST
